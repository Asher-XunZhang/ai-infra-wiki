...
(() => {
  ...
  function render(f,model,prefix=''){
    ...
    const wasOpen=target('operation-visual').querySelector('.operation-detail')?.open;
    const rankMap='<div class="wr-three">'+[0,1,2].map(r=>{
      const active=f.active.filter(id=>model.nodes[id]?.rank===r);
      return `<div class="wr-node ${active.length?'active':''}"><strong>PP${r}</strong><small>${active.map(id=>esc(model.nodes[id].title.split(' · ')[1])).join(' / ')||'本步未操作'}</small></div>`;
    }).join('')+'</div>';
    const routes=f.routes.map(w=>`<div class="wr-flow"><div class="wr-node">${esc(model.nodes[w.from].title)}</div><span class="wr-route">${({proxy:'激活',kv:'本级 KV',output:'结果',control:'控制'})[w.kind]||'消息'} →</span><div class="wr-node">${esc(model.nodes[w.to].title)}</div></div>`).join('');
    const local=!routes?`<div class="wr-node active" style="margin-top:12px">${f.active.map(id=>`<strong>${esc(model.nodes[id].title)}</strong><small>${esc(model.nodes[id].role)}</small>`).join('<br>')}</div>`:'';
    const segments=b?.segments?.length?`<div class="wr-label">${b.id} · 本批 token 区间</div><div class="wr-row">${b.segments.map(p=>`<span class="wr-token new">${p.rid}<small>[${p.start}, ${p.end})${p.last?' 最后块':''}</small></span>`).join('')}</div>`:'';
    const outcomes=s.requests.filter(q=>q.outcome!=='active').map(q=>`<div class="wr-node ${q.outcome==='success'?'active':'bad'}"><strong>${q.rid} · ${doneLabel(q.outcome)}</strong></div>`).join('');
    target('operation-visual').innerHTML=`<div class="lesson-diagram operation-focus">${rankMap}${routes}${local}${segments}${outcomes?`<div class="wr-row" style="margin-top:12px">${outcomes}</div>`:''}</div><details class="operation-detail"${wasOpen?' open':''}><summary>展开本步数据、集合与资源明细</summary>${html}</details>`;
