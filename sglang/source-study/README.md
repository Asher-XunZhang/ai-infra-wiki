...
本文是**源码分析型学习资料的目录与进度索引**，目标是从“一条请求怎样得到回答”，逐步走到调度、缓存、模型执行、多卡通信、分离部署和性能分析。
...
[机制验证练习](机制验证练习.md)连接 Pages 十二模块，分别用计时边界、共享 KV 和 PD＋PP 案例完成“预测 → 单变量操作 → 源码核对 → 明确边界”。该练习独立固定 `279339f113`，不升级下方源码课程的 `72d5c5bb` 基线。
