...
选择 M1–M5 仍可跟踪生命周期；两页跳转保留场景。“统一全程时间轴”在预设场景使用相同 0–170 u 窗口对比；自定义时间超出该范围时自动扩展，避免截掉后续清理。参数表列出本场景每级每批的实际示意 GPU 时间；指标比较最后一段 GPU 完成、全部本地清理完成及展示轮数。指标均不是 Decode 生成完成时间。
...
在任意预设上点击“基于当前场景自定义”，或在场景选择器中选择“自定义时间”，即可直接修改 GPU 表格。展开“更多阶段”还可以设置：

| 参数 | 修改范围 |
| --- | --- |
| GPU 前向 | PP0–PP2 × M1–M5，共 15 个服务时长 |
| H2D 回载 | PP0–PP2 × M2、M4，共 6 个服务时长；其他 batch 没有 host hit |
| KV 传输 | PP0–PP2 × M1–M5，共 15 个本地传输时长 |
| 消息与复制 | 激活、output、请求 / bootstrap / 终态候选传输、旧结果 D2H 各一个共用时长 |
| CPU 调度 | A–I 九个阶段分别设置相对原示例的服务时间倍率；不直接缩放依赖等待或异步设备操作 |

时间范围为 0–100 u；CPU 倍率为 0.1–10。修改后停顿约 220 ms，浏览器会重新求解依赖并更新两种视图需要的数据，包括 ACK 发布轮次、等待区间、KV 终态与实际清理名单。较慢的传输可能需要更多空 loop；计算会继续展开到全部请求释放，最多 512 轮。输入为空、越界或计算无法完成时保留上次成功结果，并显示原因，不展示未完成的生命周期。
