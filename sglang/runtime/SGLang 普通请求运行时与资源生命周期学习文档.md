...
本文是**源码分析型学习资料**，研究普通文本请求的运行时进程、对象与资源分工。沿一条普通文本请求 R1，解释入站、排队、执行、输出、取消和资源交接。交互页默认只显示当前步骤的参与者与交接，用 GPU 执行块和资源槽位表示变化；完整六泳道流程与文字说明按需展开。
...
**图意解读：** 交互页将 KV 画成固定池中的槽位；请求使用时是实色，缓存接管时改为斜纹，位置不移动。请求映射槽位单独画出。为避免重复计算，只用一个 R1、无既有共享前缀、`page_size=1`，每个有效位置在演示中只计入一种归属。真实 Radix Cache 有共享引用、重复插入与页对齐边界，不等于本图的完整资源账本。[S13]
