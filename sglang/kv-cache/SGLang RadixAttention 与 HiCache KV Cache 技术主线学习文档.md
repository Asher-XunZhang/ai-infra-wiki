...
进一步阅读：[Kimi K3 混合缓存与状态传输](<../../llm-inference/model-architecture/Kimi K3 混合注意力缓存与 Mooncake 状态传输学习文档.md>)把这条主线扩展到可变递推状态；[KV 与 MoE 权重分层内存](<../../llm-inference/kv-cache/KV Cache 与 MoE 权重的分层内存学习文档.md>)区分预取、按需访问和近内存计算。
