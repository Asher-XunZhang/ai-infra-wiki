...
(async()=>{
 ...
 await seek('player-seek',12);await page.locator('#operation-visual .operation-detail').evaluate(el=>el.open=true);await page.locator('#player-play').click();await page.clock.runFor(100);assert(await page.locator('.token.selected').first().evaluate(el=>el.getAnimations().length>0));await page.locator('#player-play').click();
