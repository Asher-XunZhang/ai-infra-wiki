"""Illustrative dependency model pinned to SGLang 72d5c5bb73.
...
def build_model(config=None):
    ...
    for r in range(3):
        ...
        for n in range(N):
            m, o = current(n), old(n)
            def cpu(name, label, phase, dur=.12, deps=(), owner='队列级', ref='pp:220'):
                nonlocal prev
                k = key(r,n,name)
            ...
            cpu('recv_req','接收/处理请求','A',.20,
                ...
                'M1–M5 请求集合' if n==0 else '后续请求集合（例中为空）','scheduler:2050')
            if r<2 and n:
            ...
            if m and r:
                cpu('recv_proxy','等待/接收上级 hidden states','D',.14,[key(r-1,n,'proxy_message')],f'M{m}','pp:852')
            ...
            if r==2 and current(n-1):
                cpu('out_work','等待上轮 output send work','F',.03,[key(0,n+1,'recv_out')],f'M{current(n-1)}','pp:708')
            elif r<2 and old(n-2):
                cpu('out_work','等待上轮 relay output send work','F',.03,[key(r+1,n-2,'recv_out')],f'M{old(n-2)}','pp:708')
            sent = m if r==2 else old(n-1)
            if sent:
                send=cpu('send_out','PP2 发 output' if r==2 else '转发上轮收到的 output','F',.10,owner=f'M{sent}',ref='pp:974')
            ...
            if r==2 or n>=3:
                ...
                cpu('send_rc','发送 release 共识回流','G',.10,owner='终态请求集合',ref='pp:681')
            if n>=2:
                ...
                cpu('recv_bc','接收 bootstrap 共识 → waiting_queue','G',.18,[bc],
                    'M1–M5 准入' if n==2 else '候选集合（本例无新准入）','pp:303')
            if r==2 or n>=3:
                peer=key(0,n+2,'recv_bc') if r==2 else key(r+1,n-1,'recv_bc')
                cpu('commit_bc','回收 bootstrap 共识发送','G',.025,[peer],ref='pp:309')
            if n>=2:
                cpu('recv_rc','接收 release 共识名单','G',.10,[rc],owner='已终态的历史请求集合',ref='pp:310')
            if r==2 or n>=3:
                peer=key(0,n+2,'recv_rc') if r==2 else key(r+1,n-1,'recv_rc')
                cpu('commit_rc','回收 release 共识发送','G',.025,[peer],ref='pp:312')
            if o:
                cpu('wait_copy','等待 d2h_event','H',.03,[key(r,n,'copy')],f'M{o}','pp:315')
                cpu('result','处理旧结果 / cache / inflight','H',.48+.08*o,owner=f'M{o}',ref='prefill:767')
                sendkv=cpu('send_kv','提交本级 KV 到 Decode','H',.22,owner=f'M{o}',ref='prefill:839')
                prev_kv=add(key(r,n,'kv'),[sendkv,prev_kv],[3.0,5.0,3.5,6.0,4.0][o-1]+r*.4,
                    r=r,n=n,kind='kv',phase='H',label=f'M{o} KV 在途',owner=f'M{o}',ref='prefill:1418')
            if n>=2:
                cpu('release','按 release 名单复查本地 poll / 清理','H',.18,owner='候选终态请求；不等于当前旧 batch',ref='prefill:918')
            if r<2:
                for name,lab in [('req','请求'),('boot','bootstrap 候选'),('term','transfer 终态候选')]:
                    snd=cpu('tail_'+name,'转发本轮'+lab,'I',.10,ref='pp:324')
                    add(key(r,n,name+'_message'),[snd],.16,r=r,n=n,kind='message',phase='I',label=lab,owner='本轮队列快照')
                if m:
                    proxy=cpu('send_proxy','wait_event → 异步发送 hidden states','I',.10,owner=f'M{m}',ref='pp:334')
                    add(key(r,n,'proxy_message'),[proxy,key(r,n,'gpu')],.26,
                        r=r,n=n,kind='message',phase='I',label=f'M{m} 激活',owner=f'M{m}')
            cpu('end','保存 output / 共识状态，结束本轮','I',.04,ref='pp:344')
    ...
    for k in resolved:
        ...
        g.update(start=round(e['start'],3),end=round(e['end'],3),ref=e.get('ref','pp:220'))
        if e.get('prev'):g['prev']=e['prev']
        extra=[]
        r,n=e['r'],e['n'];name=k.split(':')[-1]
        if 'ref' not in e:g['ref']={'out_message':'pp:974','proxy_message':'pp:338','req_message':'pp:325','boot_message':'pp:329','term_message':'pp:332','copy':'pp:1061'}.get(name,'pp:220')
