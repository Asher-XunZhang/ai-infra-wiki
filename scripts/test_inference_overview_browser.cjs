...
(async()=>{
 ...
 await go('deployment');assert.equal(await root('deployment').locator('button,input,select').count(),0);assert.match(await text('deployment'),/合并部署/);assert.match(await text('deployment'),/PD 分离/);assert.match(await text('deployment'),/三个条件要同时满足/);assert.equal(await page.locator('#deployment-comparison').evaluate(e=>e.open),false);assert.equal(await page.locator('#handoff-boundaries .wr-node').count(),4);assert.match(await page.locator('#handoff-boundaries').innerText(),/本地传输进入终态/);assert.match(await page.locator('#handoff-boundaries').innerText(),/交给后续调度/);
